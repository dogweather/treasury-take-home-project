...
. Full Python monolith (Django/FastAPI) — loses Rails productivity for UI & acceptance tests.
...
. Write only unit tests or partial integration tests — insufficient coverage of user flows.
. Write acceptance tests _after_ implementation — loses test-first behavior and verification artifacts.
. Write acceptance tests _first_ and use them as both specification and monitoring — _selected_.
...
. Code first and retro-engineer API contracts — risk misalignment.
. API contract first and derive implementation — _selected_.
